...
//!
//! Provides a database-backed implementation of CacheRepository interface
...
#[async_trait]
impl CacheRepository for DatabaseCache {
    ...
    async fn find_keys(&self, pattern: &str) -> Result<Vec<String>, RepositoryError> {
        ...
        let rows = sqlx::query(
            "SELECT DISTINCT key FROM cache WHERE key LIKE ?"
        )
        .bind(&sql_pattern)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn get_stats(&self) -> Result<CacheStats, RepositoryError> {
        ...
        let total_size_bytes: i64 = size_result.try_get("total_size").unwrap_or(0);
