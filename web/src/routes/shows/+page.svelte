...
<main class="min-h-screen bg-[#141414] pt-20 text-white">
	...
	<div class="relative z-10 bg-[#141414] pb-4">
		<div class="flex flex-wrap items-center gap-4 px-4 md:px-[60px]">
			<!-- Title -->
			<h1 class="text-2xl font-bold text-white md:text-4xl">Shows</h1>
	...
	<div class="pb-20">
		{#if viewMode === 'list'}
			...
			<div class="space-y-4">
				{#each seriesByCategory() as { name, items } (name)}
					{#if items.length > 0}
			...
			<div class="px-4 md:px-[60px]">
				...
				>
					{#each sortedSeries() as series (series.id)}
						<button
							class="series-grid-item group relative w-full text-left"
							onclick={() => openSeriesModal(series)}
							aria-label="View details for {series.title}"
						>
							<div
								class="relative aspect-video overflow-hidden rounded-md bg-gray-800 transition-all duration-200 group-hover:ring-2 group-hover:ring-white/40"
							>
								...
								>
									<div class="absolute right-0 bottom-0 left-0 p-2">
										<h3 class="line-clamp-2 text-xs font-semibold text-white">{series.title}</h3>
