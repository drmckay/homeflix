...
<section
    class="series-row relative my-4 md:my-0 hover:z-[200] z-0 transition-all duration-300 pointer-events-none"
...
>
    ...
    <div class="slider-container relative group md:-my-14">
        ...
        >
            ...
            >
                {#each items as series, index (series.id)}
                    <div class="snap-start scroll-ml-4 md:scroll-ml-[60px] pointer-events-auto">
                        <SeriesCard {series} isFirst={index === 0} isLast={index === items.length - 1} onMoreInfo={onSeriesClick} />
                    </div>
                {/each}
            </div>
        </div>
