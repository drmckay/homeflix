<script lang="ts">
	...
	let { title, items, series, onMovieClick, onSeriesClick, onPlay }: Props = $props();

	// Default handler for series click
	...
	// Find series by series_id (from the synthetic Media entry)
	function getSeriesForMedia(media: Media): Series | undefined {
		if (media.media_type === 'series' && media.series_id) {
			return series.find((s) => s.id === media.series_id);
		}
		return undefined;
	}
...
<section
	class="movie-row pointer-events-none relative z-0 my-4 transition-all duration-300 hover:z-[200] md:my-0"
...
>
	...
	<div class="slider-container group relative md:-my-14">
		...
		>
			<!-- Track -->
			<div class="slider-track flex w-max gap-2 py-4 md:py-16">
