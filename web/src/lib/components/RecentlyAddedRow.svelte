<script lang="ts">
    ...
    function getSeriesForMedia(media: Media): Series | undefined {
        if (media.media_type === 'series' && media.series_id) {
            return seriesMap().get(media.series_id);
        }
        return undefined;
    }
...
>
    ...
    <div class="slider-container relative group" onwheel={handleWheel}>
        ...
        >
            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 md:h-10 md:w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
